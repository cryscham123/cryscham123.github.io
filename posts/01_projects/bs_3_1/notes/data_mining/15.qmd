...
[Table 4,5](#fig:survey-category)의 범주를 기반으로 설문 문항들을 요인 분석하여 도출된 심리 요인 점수를 독립 변수로 활용하였다. 본 연구에서는 부모애착 정도, 일탈적 자아 낙인, 부모에 의한 스트레스, 부모 감독, 물질적인 요소로 인한 스트레스, 친구로 인한 스트레스, 자기신뢰감, 상급학교 의존도, 부정적 자아존중감, 학업으로 인한 스트레스의 총 10개 요인을 선정하였다.

각 연차별로 수행된 요인 분석 모델에 대한 적합성 평가를 진행하였다. 이는 설정된 잠재 요인 구조가 실제 관측된 데이터의 패턴과 얼마나 잘 일치하는지를 나타낸다. 이 지표들이 양호한 값을 보일수록, 가정한 심리 요인들이 실제 청소년들의 응답 패턴을 어느 정도 잘 반영하고 있다고 해석될 수 있다. 자세한 결과는 [Table 6](#fig:cfa-result)과 같다.
...
이후, 분석에 사용될 최종 변수들에 대해 결측치가 없는 행을 제거하여 최종 분석 데이터셋을 구축하였다. 종속 변수는 원 데이터의 상세 분류 코드를 바탕으로 2개의 범주로 그룹화하고 팩터 형태로 변환하였다. 종속 변수의 분류는 [Table 7](#fig:dependent_variable_groups)과 같다.
