---
title: 청소년기의 심리·정서적 요인이 대학 비진학 청년의 조기 경제활동 참여에 미치는 영향 분석
categories: ["보고서", "데이터 마이닝"]
mainfont: "Noto Sans KR"
sansfont: "Noto Sans KR"
monofont: "Noto Sans KR"
date: last-modified
geometry:
  - top=25mm
  - left=25mm
  - right=25mm
  - bottom=25mm
format: 
  titlepage-pdf:
    documentclass: scrartcl
    classoption: ["oneside", "open=any", "twocolumn"]
    number-sections: true
    toc: false
    titlepage: "none"
    coverpage: "none"
    fontsize: 11pt
    titlepage-include-file:
      - _tex/title.tex
    include-in-header: 
      text: |
        \usepackage[utf8]{inputenc}
        \usepackage{graphicx}
        \usepackage{amsmath}
        \usepackage{amsfonts}
        \usepackage{amssymb}
        \usepackage{geometry}
        \usepackage{fancyhdr}
        \usepackage{hyperref}
        \usepackage{lipsum}
        \setlength{\columnsep}{15mm}
        \usepackage{setspace}
        \setstretch{1.2}
        \usepackage{booktabs}
        \usepackage{tabularx}
        \usepackage{makecell}
        \usepackage{caption}
        \usepackage{multirow}
        \usepackage{array}
        \usepackage{graphicx}
        \renewcommand{\arraystretch}{1.3}
        \renewcommand{\tabularxcolumn}[1]{m{#1}}

execute:
  echo: false
  warning: false
  message: false
---

# 서론

추가 예정

# 관련 연구

추가 예정

# 연구 방법

## 데이터 개요

본 연구는 청소년기 심리·정서적 요인이 성인기 초기 진로 이행에 미치는 영향을 예측적으로 분석하고자 한국청소년패널조사(KYPS)의 중2 패널 1차 년도(2003년)부터 6차 년도(2008년)까지의 자료를 활용하였다.

초기 1차년도 조사의 최종 확보 표본은 3,449명이었으나, 6차년도까지 모든 조사에 응답하고 본 연구의 분석에 필요한 주요 변수들에서 결측치가 없는 개체를 선별한 결과, 최종 분석 대상은 2,451명의 데이터로 구성되었다.

각 연차별 데이터는 연구 대상에 대한 5점 리커트 척도의 설문 문항 점수를 저장하고 있다. 설문 문항은 [Table 4,5](#fig:survey-category)와 같은 주요 범주로 구성되어 있으며, 연차 별로 총 560개 이상의 문항이 포함되어 있다.

## 요인 분석

[Table 4,5](#fig:survey-category)의 범주를 기반으로 설문 문항들을 요인 분석하여 도출된 심리 요인 점수를 독립 변수로 활용하였다. 본 연구에서는 부모애착 정도, 일탈적 자아 낙인, 부모에 의한 스트레스, 부모 감독, 물질적인 요소로 인한 스트레스, 친구로 인한 스트레스, 자기신뢰감, 상급학교 의존도, 부정적 자아존중감, 학업으로 인한 스트레스의 총 10개 요인을 선정하였다. 요인 분석의 결과는 [Table 6](#fig:cfa-result)과 같다.

각 연차별로 수행된 CFA 모델에 대한 적합성 평가는 설정된 잠재 요인 구조가 실제 관측된 데이터의 패턴과 얼마나 잘 일치하는지를 나타낸다. 이 지표들이 양호한 값을 보일수록, 가정한 심리 요인들이 실제 청소년들의 응답 패턴을 어느 정도 잘 반영하고 있다고 해석될 수 있다.

CFI(Comparative Fit Index)와 TLI(Tucker-Lewis Index)는 대부분의 연차에서 0.90 이상을 기록하며 양호한 수준을 보였다. 이는 설정된 요인 구조가 데이터의 변동을 설명하는 데 있어 기준 모델 대비 비교적 적절함을 시사한다.

RMSEA(Root Mean Square Error of Approximation) 지수는 모든 연차에서 0.08 이하로 나타났다. 이 지표는 모델이 모집단 공분산 구조와 얼마나 근사적인 차이를 보이는지를 나타내며, 값이 0에 가까울수록 더 나은 적합성을 의미한다.

SRMR(Standardized Root Mean Square Residual) 지수 또한 모든 연차에서 0.08 이하로 나타났다. 이 지표는 관측된 상관과 모델에 의해 예측된 상관 간의 평균적인 표준화 차이를 나타내며, 값이 0에 가까울수록 모델의 예측이 실제와 가깝다고 해석된다.

연차별 적합도 지표 값에 다소 변동이 있었으나, 전반적으로 6년에 걸쳐 분석된 심리 요인 모델들은 데이터 패턴에 대해 비교적 일관되게 양호한 수준의 부합성을 보이는 것으로 판단된다. 이러한 적합도 결과는 본 연구에서 설정한 심리 요인 구조가 6년의 추적 기간 동안 청소년들의 심리 관련 응답 패턴을 나타내는 데 있어 어느 정도 타당한 측정 모델로서 활용될 수 있음을 지지하는 근거가 될 수 있다.

## 데이터 전처리

먼저, 연도별로 분리된 패널 데이터를 개인 식별자를 기준으로 횡 형태로 통합하였다. 통합된 데이터에서 본 연구에 필요한 독립 변수, 종속 변수, 횡, 종단 가중치의 변수 컬럼들을 추출하였다. 이후, 분석에 사용될 최종 변수들에 대해 결측치가 없는 행을 제거하여 최종 분석 데이터셋을 구축하였다. 종속 변수는 원 데이터의 상세 분류 코드를 바탕으로 2개의 범주로 그룹화하고 팩터 형태로 변환하였다. 종속 변수의 분류는 [Table 7](#fig:dependent_variable_groups)과 같다.

## 데이터 분포

최종 분석에 활용된 대상자의 인구사회학적 특성 및 종속 변수의 분포는 다음의 표의 내용과 같다.

\begin{table}[h]
\centering
\caption{출생년도별 분포 (조사 시점: 2008년)}
\label{tab:birthyear-freq} % 고유한 레이블로 변경
\begin{tabularx}{\columnwidth}{@{} *{3}{>{\centering\arraybackslash}X} @{}}
% 너비를 \columnwidth로, 열 개수를 3개로 수정
\toprule
출생년도 & 빈도 & 비율 \\
\midrule
88 & 2 & 0.08 \\
\midrule
89 & 1895 & 77.32 \\
\midrule
90 & 554 & 22.60 \\ % 마지막 행 뒤에는 \\를 붙이지 않는 것이 일반적입니다.
\bottomrule
\end{tabularx}
\end{table}

\begin{table}[h]
\centering
\caption{성별 분포}
\label{tab:gender-freq} % 고유한 레이블로 변경
\begin{tabularx}{\columnwidth}{@{} *{3}{>{\centering\arraybackslash}X} @{}}
% 너비를 \columnwidth로, 열 개수를 3개로 수정
\toprule
성별 & 빈도 & 비율 \\
\midrule
남성 & 1165 & 47.53 \\
\midrule
여성 & 1286 & 52.47 \\
\bottomrule
\end{tabularx}
\end{table}

\begin{table}[h]
\centering
\caption{종속 변수 분포}
\label{tab:dependentvar-freq} % 고유한 레이블로 변경
\begin{tabularx}{\columnwidth}{@{} *{3}{>{\centering\arraybackslash}X} @{}}
% 너비를 \columnwidth로, 열 개수를 3개로 수정
\toprule
구분 & 빈도 & 비율 \\
\midrule
생산적활동참여 & 102 & 4.16 \\
\midrule
\makecell[lt]{비 경제활동 및 \\ 학업 관련 상태} & 2349 & 95.84 \\
\bottomrule
\end{tabularx}
\end{table}

먼저, 조사 시점인 2008년을 기준으로 대상자들의 출생년도를 살펴본 결과, 대부분의 대상자가 1989년(77.32%) 또는 1990년(22.60%)생으로 나타났다. 이는 1차 조사 시작 시점(2003년)에 중학교 2학년이었던 표본이 6차년도 시점에 대략 만 18세(1990년생) 또는 만 19세(1989년생)에 해당함을 보여준다. 소수의 대상자(0.08%)는 만 20세로 확인되었다.

성별 분포에서는 남성(47.53%)이 1,165명, 여성(52.47%)이 1,286명으로, 전체적으로 남성보다 여성의 비율이 약간 더 높은 것으로 나타났다.

본 연구에서 최종적으로 분석에 사용된 종속 변수의 분포는 '생산적 활동 참여' 범주에 해당하는 대상자가 102명(4.16%)이었으며, '비 경제활동 및 학업 관련 상태' 범주에 해당하는 대상자는 2,349명(95.84%)으로 나타났다.

## 독립 변수 간 상관관계

최종 데이터셋에서 사용된 독립 변수 간의 상관관계는 아래 [히트맵](#fig:var-heatmap)과 같은 양상을 보인다.
전반적인 상관 관계 패턴: 히트맵을 통해 파악된 전반적인 관계 패턴에 대해 서술합니다. (예: "대체로 동일 요인의 시점 간 요인 점수들은 강한 정적 상관 관계를 보였으며, 특정 다른 요인들 간에도 연차에 걸쳐 유의미한 상관 관계가 관찰되었다.")

주요 상관 관계 강조: 제시하신 |r| > 0.7인 변수 쌍들을 포함하여, 분석적으로 중요하거나 특히 상관이 높은 몇 가지 변수 쌍을 선별하여 구체적인 상관계수 값과 함께 제시합니다. (이때는 표 형태로 제시하는 것보다 텍스트로 자연스럽게 서술하는 것이 좋습니다.)

다중공선성 언급: 이러한 상관 관계가 독립 변수들 간에 어느 정도의 다중공선성이 존재함을 시사하며, 이는 모델 해석 시 주의해야 할 부분임을 언급합니다. (하지만 사용하신 Random Forest 모델은 다중공선성에 비교적 덜 민감하다는 점도 함께 언급 가능).

\begin{table*}[h!]
\centering
\caption{원 데이터 설문 문항 분류}
\label{fig:survey-category}
\begin{tabularx}{\textwidth}{lX}
\toprule
대분류 & 소분류 \\
\midrule
인적사항 & 성별 \newline
출생년월 \newline
학교위치 \newline
거주 지역 \\
\midrule
희망 직업 &
직업성숙도 \newline
장래희망직업선정여부 \newline
장래희망직업 \newline
장래희망직업을 갖고싶게된계기 \newline
장래희망직업선택시기 \newline
장래희망직업의 특성 \newline
희망직업 선택의 중요요인 \newline
희망직업생활의 성공 요인 \newline
희망직업성취의 중요 요인 \newline
희망직업성취에 필요한 학력 \newline
희망직업의 주관적 성취 여부 \newline
부모님께서 바라시는 직업 \\
\midrule
\makecell[lt]{향후 진로 \\ 계획} & % makecell을 사용하여 '대분류' 항목도 여러 줄 처리, [lt]는 왼쪽-상단 정렬
학업중단의 원인 \newline
전학, 휴학 및 중퇴의사 \newline
중학교 졸업후의 진로설정 \newline
휴학, 중퇴후의 진로 설정 \newline
향후진로설정과관련된생각 \newline
진로성숙도 \newline
학생이 원하는교육수준 \newline
부모님이 원하는 교육수준 \newline
향후진로설정관련부모님과의대화여부 \newline
향후진로설정의계기 \\
\midrule
\makecell[lt]{진로 준비 \\ 현황} &
성적 \newline
학교수업참여 \newline
사교육 참여 \newline
개인공부 \newline
수상및자격증취득경력 \newline
진로준비 활동 \newline
아르바이트 경험 유무 \newline
아르바이트 직종 \newline
아르바이트 참여 실태 \newline
아르바이트 참여 이유 \newline
아르바이트 구직 기준 \newline
아르바이트 도중 부당대우 경험 정도 \newline
아르바이트 참여시 근로계약서 작성 여부 \newline
아르바이트 참여시 부모동의서 작성 여부 \newline
아르바이트 경험의 미래 직업생활 도움 여부 \\
\bottomrule
\end{tabularx}
\end{table*}

\begin{table*}[h!]
\centering
\caption{원 데이터 설문 문항 분류}
\begin{tabularx}{\textwidth}{lX}
\midrule
\makecell[lt]{여가 및 사회 \\ 활동} &
여가종목 \newline
여가를 함께보내는사람 \newline
여가를 보내는장소 \newline
여가만족도 \newline
여가제약 \newline
수련활동 참여실태 \newline
자원봉사활동 참여 실태 \newline
동아리 활동참여실태 \\
\midrule
\makecell[lt]{일탈 및 \\ 피해 경험} &
부모애착 \newline
부모감독 \newline
자아통제 \newline
공부압력 \newline
선생님 애착 \newline
학교친구애착 \newline
친한친구와의접촉빈도 \newline
친한친구애착 \newline
친한친구들의일탈경험 \newline
본인의 일탈경험 \newline
본인의 인터넷일탈경험 \newline
한달평균용돈 \newline
용돈의 출처 \newline
컴퓨터의 주된이용장소 \newline
하루평균컴퓨터이용시간 \newline
컴퓨터 이용의목적 \newline
휴대전화 보유여부 \newline
하루평균휴대전화사용시간 \newline
휴대전화 요금 \newline
휴대전화 이용목적 \\
\midrule
\makecell[lt]{생활 영역별 \\ 시간배분 \\ 및 중요도} &
생활영역별 시간배분 \newline
생활영역별 중요도 \newline
친한사람의 유형 \\
\midrule
\makecell[lt]{자아관 관련 \\ 심리 문항} &
긍정적 자아존중감 \newline
부정적 자아존중감 \newline
스스로의 일탈적 자아낙인 \newline
주위사람들의일탈적자아낙인 \newline
주위사람들에대한애착 \newline
자기신뢰감 \newline
공격성 \newline
신체적 공격성 \newline
분노 \newline
정서적 조절감 \newline
성역할정체감 \newline
부모님에 의한 스트레스 \newline
학업으로 인한 스트레스 \newline
친구로 인한 스트레스 \newline
외관으로 인한 스트레스 \newline
물질적인 요소로 인한 스트레스 \newline
삶에대한전반적만족도 \\
\bottomrule
\end{tabularx}
\end{table*}

\begin{table*}[t]
\centering
\caption{Confirmatory Factor Analysis 결과}
\label{fig:cfa-result}
\begin{tabularx}{\textwidth}{@{} *{5}{>{\centering\arraybackslash}X} @{}}
\toprule
년차 & CFI & TLI & RMSEA & SRMR \\
\midrule
1 & 0.929 & 0.918 & 0.048 & 0.042 \\
\midrule
2 & 0.943 & 0.934 & 0.047 & 0.037 \\
\midrule
3 & 0.937 & 0.927 & 0.048 & 0.043 \\
\midrule
4 & 0.927 & 0.915 & 0.054 & 0.046 \\
\midrule
5 & 0.935 & 0.924 & 0.052 & 0.045 \\
\midrule
6 & 0.906 & 0.891 & 0.064 & 0.059 \\
\bottomrule
\end{tabularx}
\end{table*}

\begin{table*}[t]
\centering
\caption{종속 변수 그룹}
\label{fig:dependent_variable_groups} % 레이블을 tab:으로 시작하는 것이 일반적입니다.
% X 열의 내용을 왼쪽 정렬하고 싶다면 다음 줄의 주석을 해제하세요.
% \renewcommand{\tabularxcolumn}[1]{>{\raggedright\arraybackslash}p{#1}}
\begin{tabularx}{\textwidth}{lX}
\toprule
종속 변수 & 원 데이터 값 \\
\midrule
생산적활동참여 & 고등학교는졸업하였고현재취업(임시직, 일용직포함)해있는상태이다 \newline
고등학교는졸업하였고현재창업해있는상태이다 \newline
고등학교는졸업하였고현재무보수로가업을돕고있다 \newline
현재취업(임시직, 일용직포함)상태에서대학생활(방통대, 야간대등)도 병행하고있다 \newline
현재창업상태에서대학생활(방통대, 야간대등)도병행하고있다 \newline
현재무보수로가업을돕고있는상태에서대학생활(방통대, 야간대등)도 병행하고있다 \\
\midrule
비경제활동및학업관련상태 & 대학에 진학하였고 졸업할 때까지 계속 다닐 생각이다 \newline
대학에 진학하였고 조만간에 휴학할 생각이다 \newline
대학에 진학하였고 현재 휴학중이다 \newline
대학에 진학하였고 조만간에 그만둘(중퇴할) 생각이다 \newline
대학에 진학한 후 중퇴했으며 현재 다른 대학 입학을 준비중이다 \newline
고등학교는 졸업하였고 현재 대학 입학을 준비중이다 \newline
고등학교는 졸업하였고 현재 취업(임시직, 일용직포함)을 위해 노력중이다 \newline
고등학교는 졸업하였고 현재 창업을 준비하고있다 \newline
대학에 진학한 후 중퇴했으며 현재 취업(임시직, 일용직포함)을 위해 노력중이다 \newline
대학에 진학한 후 중퇴했으며 현재 창업을 준비하고있다 \newline
고등학교는 졸업하였고 현재 가사,육아를 돕고있다 \newline
고등학교는 졸업하였고 현재 군 입대 대기중이다 \newline
고등학교는 졸업하였고 현재 아무일도 하지 않고있다 \newline
고등학교 재학중 \newline
중학교 재학중 \newline
고등학교, 중학교 졸업 전 중퇴 \\
\bottomrule
\end{tabularx}
\end{table*}

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{correlation_heatmap.png}
    \caption{독립변수 히트맵}
    \label{fig:var-heatmap}
\end{figure*}

\clearpage

# 연구 과정

Random forest 씀. 다른 방법도 써야하나? 모르겠다. 내용은 추가 예정.

# 연구의 결과

추가 예정

# 결론

1. 표본 갯수 적음
1. 종속변수의 분포가 불균형함

# 참고문헌

- [한국 아동 청소년 청년 데이터 아카이브](https://www.nypi.re.kr/archive/board?menuId=MENU00219)
