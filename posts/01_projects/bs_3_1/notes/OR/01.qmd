...
# 초기 설정
obj = [-10, -4, -5, -7, 0, 0, -600]
A = [
    [2, 3, 4, 2, 1, 0, 300],
    [8, 1, 1, 5, 0, 1, 300]
]
basic = np.array([5, 6]) - 1
non_basic = np.array([1, 2, 3, 4]) - 1
...
basic = np.array([5, 7]) - 1  # x5, x6
non_basic = np.array([1, 2, 3, 4, 6]) - 1  # x1, x2, x3, x4
...
model.setObjective(4 * x1 + 2 * x2 + 3 * x3 + 5 * x4, GRB.MAXIMIZE)

model.addConstr(2 * x1 + 3 * x2 + 4 * x3 + 2 * x4 == 300)
model.addConstr(8 * x1 + x2 + x3 + 5 * x4 == 300)
